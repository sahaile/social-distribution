...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadMenu (currentAuthor) {
    ...
    homeButton.style.borderRight = '3px solid #0cc0df'
    homeButton.style.backgroundColor = '#f2f2f2ff'
    ...
    const postToggleButton = document.getElementById('post-toggle-button')
    const postMenuButton = document.getElementById('post-menu-button')
    const modal = document.querySelector('.post-tweet-modal')

    const openModal = () => {
      modal.style.display = 'flex'
    }

    postToggleButton.addEventListener('click', openModal)
    postMenuButton.addEventListener('click', openModal)
